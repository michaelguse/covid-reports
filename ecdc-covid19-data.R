...
data <- data %>% 
	rename(country_name = `Countries and territories`, 
		country_code = GeoId, 
		date = DateRep, 
		day = Day, 
		year = Year, 
...
p1 <- ggplot(data = filter(covid19, country_code == "US")[1:co_days,], mapping = aes(x = date, y = cases)) +
...
p2 <- ggplot(data = filter(covid19, country_code == "US")[1:co_days,], mapping = aes(x = date, y = deaths), color = "black") +	
	geom_bar(stat = "identity", color = "black", fill = "black") +
	labs(title="DE Daily Deaths", x = "Date", y = "Daily Death Count") +
	labs(title="US Daily Deaths", x = "Date", y = "Daily Death Count") +
	theme(plot.title = element_text(size=14, face="bold", margin = margin(10,0,10,0)))

p3 <- ggplot(data = filter(covid19, country_code == "DE")[1:co_days,], mapping = aes(x = date, y = cases)) +
	geom_bar(stat = "identity", color = "red", fill = "red") +
	labs(title="DE Daily Cases", x = "Date", y = "Daily Case Count") +
	theme(plot.title = element_text(size=14, face="bold", margin = margin(10,0,10,0)))
	
p4 <- ggplot(data = filter(covid19, country_code == "DE")[1:co_days,], mapping = aes(x = date, y = deaths), color = "black") +	
	geom_bar(stat = "identity", color = "black", fill = "black") +
	labs(title="DE Daily Deaths", x = "Date", y = "Daily Death Count") +
	theme(plot.title = element_text(size=14, face="bold", margin = margin(10,0,10,0)))

p5 <- ggplot(data = covid19_all, mapping = aes(x = date, y = sum_cases)) +
	geom_bar(stat = "identity", color = "red", fill = "red") +
	labs(title="Global Daily Cases", x = "Date", y = "Daily Case Count") +
	theme(plot.title = element_text(size=14, face="bold", margin = margin(10,0,10,0)))
	
p6 <- ggplot(data = covid19_all, mapping = aes(x = date, y = sum_deaths), color = "black") +	
	geom_bar(stat = "identity", color = "black", fill = "black") +
	labs(title="Global Daily Deaths", x = "Date", y = "Daily Death Count") +
	theme(plot.title = element_text(size=14, face="bold", margin = margin(10,0,10,0)))

p7 <- ggplot(data = filter(covid19, country_code == "IT")[1:co_days,], mapping = aes(x = date, y = cases)) +
	geom_bar(stat = "identity", color = "red", fill = "red") +
	labs(title="IT Daily Cases", x = "Date", y = "Daily Case Count") +
	theme(plot.title = element_text(size=14, face="bold", margin = margin(10,0,10,0)))
	
p8 <- ggplot(data = filter(covid19, country_code == "IT")[1:co_days,], mapping = aes(x = date, y = deaths), color = "black") +	
	geom_bar(stat = "identity", color = "black", fill = "black") +
